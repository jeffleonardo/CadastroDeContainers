	...
	<div class="externa">
		...
		<div class="interna">
			<form name="newContainer" action="updateC" method="get">
				...
				<h4>Cliente:</h4>
				<input type="text" class="caixa2" name="nomeCliente" required
					value="<%out.print(request.getAttribute("nomeCliente"));%>">
				<br>
				<h4>Número do container:</h4>
				<input type="text" class="caixa2" pattern="[a-zA-Z]{4}[0-9]{7}"
					name="numContainer" required
					value="<%out.print(request.getAttribute("numContainer"));%>">
				<br>

				<h4>Tipo de container:</h4>
				<select name="tipo" class="caixa2" required
				...
				<h4>Status:</h4>
				<select name="status" class="caixa2" required
				...
				<h4>Categoria:</h4>
				<select name="categoria" class="caixa2" required
					value="<%out.print(request.getAttribute("categoria"));%>">
					<option value="Importacao">Importação</option>
					<option value="Exportacao">Exportacao</option>
				</select><br>
				<h4>Movimentação:</h4>
				<select name="movimentacao" class="caixa2" required
					value="<%out.print(request.getAttribute("movimentacao"));%>">
				...
				<input type="datetime-local" name="dataInicio"
					value="<%out.print(request.getAttribute("dataInicio").toString().replace(" ", "T").substring(0, 19 - 3));%>"
					class="caixa2">
